...
namespace WCMS.Common.Utilities
{
    ...
    public abstract class WebUtil
    {
        ...
        public static void considerUsing()
        {
            // using Microsoft.AspNetCore.Mvc.Rendering;

            // Create an instance of TagBuilder for an anchor element
            var link = new TagBuilder("a");
            link.Attributes["href"] = "https://www.example.com";
            link.InnerHtml.Append("Click here");

            // Optionally, set additional attributes
            link.Attributes["target"] = "_blank"; // Open link in a new tab

            // Render the link to a string
            using (var writer = new StringWriter())
            {
                link.WriteTo(writer, HtmlEncoder.Default);
                string htmlString = writer.ToString();
                // Use the htmlString as needed
            }
