...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_AuditLog_LogDateTime]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[EventLog] ADD  CONSTRAINT [DF_AuditLog_LogDateTime]  DEFAULT (getdate()) FOR [EventDate]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_AuditLog_Content]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[EventLog] ADD  CONSTRAINT [DF_AuditLog_Content]  DEFAULT ('') FOR [Content]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_EventLog_UserId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[EventLog] ADD  CONSTRAINT [DF_EventLog_UserId]  DEFAULT ((-1)) FOR [UserId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_EventLog_Action]') AND type = 'D')
BEGIN
