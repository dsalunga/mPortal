...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_FileVersion_VersionDate]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[FileVersion] ADD  CONSTRAINT [DF_FileVersion_VersionDate]  DEFAULT (getdate()) FOR [VersionDate]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_FileVersion_Activity]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[FileVersion] ADD  CONSTRAINT [DF_FileVersion_Activity]  DEFAULT ((0)) FOR [Activity]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_FileVersion_UserId]') AND type = 'D')
BEGIN
