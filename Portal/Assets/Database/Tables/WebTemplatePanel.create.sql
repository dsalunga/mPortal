...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebTemplatePanels_TemplateId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTemplatePanel] ADD  CONSTRAINT [DF_WebTemplatePanels_TemplateId]  DEFAULT ((-1)) FOR [TemplateId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebTemplatePanel_Rank]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTemplatePanel] ADD  CONSTRAINT [DF_WebTemplatePanel_Rank]  DEFAULT ((0)) FOR [Rank]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebTempla__ObjectId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTemplatePanel] ADD  CONSTRAINT [DF__WebTempla__ObjectId]  DEFAULT ((-1)) FOR [ObjectId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebTempla__RecordId]') AND type = 'D')
BEGIN
