...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebRoles_IsSystem]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebRole] ADD  CONSTRAINT [DF_WebRoles_IsSystem]  DEFAULT ((0)) FOR [IsSystem]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebRoles_ParentId]') AND type = 'D')
BEGIN
