...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_BibleReaderAccess_AppAccessCount]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[BibleReaderAccess] ADD  CONSTRAINT [DF_BibleReaderAccess_AppAccessCount]  DEFAULT ((-1)) FOR [AppAccessCount]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_BibleReaderAccess_LastAccess]') AND type = 'D')
BEGIN
