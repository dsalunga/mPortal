...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebConsta__Category]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebConstant] ADD  CONSTRAINT [DF__WebConsta__Category]  DEFAULT ('') FOR [Category]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebConsta__ObjectId]') AND type = 'D')
BEGIN
