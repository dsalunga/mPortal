...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_USState_StateName]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[CountryState] ADD  CONSTRAINT [DF_USState_StateName]  DEFAULT ('') FOR [StateName]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_USState_ZipCode]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[CountryState] ADD  CONSTRAINT [DF_USState_ZipCode]  DEFAULT ('') FOR [ZipCode]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_USState_CountryCode]') AND type = 'D')
BEGIN
