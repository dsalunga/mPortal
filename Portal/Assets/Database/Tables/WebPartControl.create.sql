...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebPartControl_AdminPartId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebPartControl] ADD  CONSTRAINT [DF_WebPartControl_AdminPartId]  DEFAULT ((-1)) FOR [PartAdminId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebPartCo__Entry]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebPartControl] ADD  CONSTRAINT [DF__WebPartCo__Entry]  DEFAULT ((1)) FOR [EntryPoint]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebPartControl_ParentId]') AND type = 'D')
BEGIN
