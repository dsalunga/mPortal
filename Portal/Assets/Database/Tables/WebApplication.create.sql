...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebApplication_AppKey]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebApplication] ADD  CONSTRAINT [DF_WebApplication_AppKey]  DEFAULT ('') FOR [AppKey]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebApplication_IpAddresses]') AND type = 'D')
BEGIN
