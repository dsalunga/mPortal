...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_Menu_RefPageId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[Menu] ADD  CONSTRAINT [DF_Menu_RefPageId]  DEFAULT ((-2)) FOR [PageId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_Menu_IncludeChildren]') AND type = 'D')
BEGIN
