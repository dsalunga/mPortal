...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_Wall_Content]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WallUpdate] ADD  CONSTRAINT [DF_Wall_Content]  DEFAULT ('') FOR [Content]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_Wall_UpdateDate]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WallUpdate] ADD  CONSTRAINT [DF_Wall_UpdateDate]  DEFAULT (getdate()) FOR [UpdateDate]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_Wall_EventTypeId]') AND type = 'D')
BEGIN
