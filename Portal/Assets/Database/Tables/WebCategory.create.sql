...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebCategor__Name]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebCategory] ADD  CONSTRAINT [DF__WebCategor__Name]  DEFAULT ((-1)) FOR [Name]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebCatego__ObjectId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebCategory] ADD  CONSTRAINT [DF__WebCatego__ObjectId]  DEFAULT ((-1)) FOR [ObjectId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebCategor__Rank]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebCategory] ADD  CONSTRAINT [DF__WebCategor__Rank]  DEFAULT ((0)) FOR [Rank]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebCatego__ParentId]') AND type = 'D')
BEGIN
