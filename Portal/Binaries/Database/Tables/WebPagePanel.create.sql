...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebPagePanels_PageId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebPagePanel] ADD  CONSTRAINT [DF_WebPagePanels_PageId]  DEFAULT ((-1)) FOR [PageId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebPagePanels_UsageTypeId]') AND type = 'D')
BEGIN
