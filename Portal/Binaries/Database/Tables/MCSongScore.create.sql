...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__MCSongS__DateModified]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[MCSongScore] ADD  CONSTRAINT [DF__MCSongS__DateModified]  DEFAULT (getdate()) FOR [DateModified]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__MCSongS__CandidateId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[MCSongScore] ADD  CONSTRAINT [DF__MCSongS__CandidateId]  DEFAULT ((-1)) FOR [CandidateId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__MCSongS__CompetitionId]') AND type = 'D')
BEGIN
