...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebShortUrl_Name]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebShortUrl] ADD  CONSTRAINT [DF_WebShortUrl_Name]  DEFAULT ('') FOR [Name]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebShortUrl_PageId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebShortUrl] ADD  CONSTRAINT [DF_WebShortUrl_PageId]  DEFAULT ((-1)) FOR [PageId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebShortU_PageUrl]') AND type = 'D')
BEGIN
