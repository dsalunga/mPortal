...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebGlobalPolicy_GlobalPolicyId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebGlobalPolicy] ADD  CONSTRAINT [DF_WebGlobalPolicy_GlobalPolicyId]  DEFAULT ((-1)) FOR [GlobalPolicyId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebGlobalPolicy_Name]') AND type = 'D')
BEGIN
