...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebTheme_TemplateId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTheme] ADD  CONSTRAINT [DF_WebTheme_TemplateId]  DEFAULT ((-1)) FOR [TemplateId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_WebTheme_Name]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTheme] ADD  CONSTRAINT [DF_WebTheme_Name]  DEFAULT ('') FOR [Name]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebTheme__ParentId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTheme] ADD  CONSTRAINT [DF__WebTheme__ParentId]  DEFAULT ((-1)) FOR [ParentId]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebTheme__Identiy]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[WebTheme] ADD  CONSTRAINT [DF__WebTheme__Identiy]  DEFAULT ('') FOR [Identity]
END

GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF__WebTheme__SkinId]') AND type = 'D')
BEGIN
