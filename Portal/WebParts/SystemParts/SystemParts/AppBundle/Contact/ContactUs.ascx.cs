...
namespace WCMS.WebSystem.WebParts.Contact
{
    ...
    /// </summary>
    public partial class ContactController : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (formState != null)
                {
                    lblThanks.InnerHtml = WebRegistry.SelectNode(ContactConstants.ThankYouMsgPath).Value;
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(sendToXml))
                    {
                        ...
                        XmlNodeList nodes = xdoc.SelectNodes("//SendTo");
        ...
        {
            if (rowCode.Visible && txtVerificationCode.Text != ImageSecurity1.Text)
            {
            ...
            {
                ContactLink link = ContactLink.Get(context.ObjectId, context.RecordId);
            ...
            string addressLine2 = ""; // txtAddressLine2.Text.Trim();
            string sCity = ""; // txtCity.Text.Trim();
            //string sCountry = ""; // cboCountry.SelectedItem == null ? string.Empty : cboCountry.SelectedItem.Text.Trim();
            int countryCode = -1; // int.Parse(cboCountry.SelectedValue);
            //string sState = ""; // cboState.SelectedItem.Text;
            int stateCode = -1; // int.Parse(cboState.SelectedValue);
            string zipCode = ""; // txtZipCode.Text.Trim();
            string fax = ""; // txtFax.Text.Trim();

            string inquiryType = ""; // cboInquiryType.SelectedValue;

            // VALIDATE INPUT HERE
            //if (iCountryCode == -1)
            //{
            //    lblMessage.Text = "Please select your country";
            //    return;
            //}

            //if (iStateCode == -1)
            //{
            //    lblMessage.Text = "Please select your state";
            //    return;
            //}
            // END OF VALIDATION


            //int contactId = -1; // DataHelper.GetId(cboSendTo.SelectedValue);

            //Contact contact = null;
            //if (contactId > 0 && (contact = Contact.Get(contactId)) != null)
            //{
            //    sSendToEmail = contact.Email;
            //    sEmailSubject = contact.Subject;
            //    sSendToName = contact.Name;
            //}
            ...
            if (emails.Count > 0)
            {
                try
                {
                    ...
                    WebMailMessage m = new WebMailMessage();
        ...
        private void SendReply(string sSubject, string to, string from, string sFullname, string msg)
        {
            // REPLY HERE
