...
namespace WCMS.WebSystem.Apps.Integration.MusicCompetition
{
    public partial class MessageBoardV2 : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                var objectId = element.GetParameterValue("ObjectId", element.OBJECT_ID.ToString());
                var recordId = element.GetParameterValue("RecordId", element.Id.ToString());

                hObjectId.Value = objectId;
                hRecordId.Value = recordId;
            }
        }
