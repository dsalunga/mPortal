    ...
    public class ExternalHelper
    {
        ...
        public static LogAttendanceResult LogAttendance(int memberId, int serviceScheduleId)
        {
            ...
            if (serviceSchedule != null &&
                attendanceClient.Insert3((long)memberId, serviceSchedule.LocaleID, serviceScheduleId,
                    serviceSchedule.ServiceID, serviceSchedule.WeekNo,
                    DateTime.Now.ToString("u"), serviceSchedule.LateDateTime.ToString("u"),
                    ExternalAttendanceStatus.Normal, true))
                result.Status = 200;
            return result;
        }
        ...
        public static LogAttendanceResult LogAttendance(int memberId, int serviceScheduleId, DateTime timeIn, string status)
        {
            ...
            if (serviceSchedule != null &&
                attendanceClient.Insert3((long)memberId, serviceSchedule.LocaleID, serviceScheduleId,
                    serviceSchedule.ServiceID, serviceSchedule.WeekNo,
                    timeIn.ToString("u"), serviceSchedule.LateDateTime.ToString("u"),
                    status, true))
                result.Status = 200;
            return result;
        }

        public static IEnumerable<MemberAttendance> GetAttendanceList(DateTime serviceStartDate, DateTime serviceEndDate,
            int memberID = -1, int localeID = -1, int localeGroupID = -1)
        {
