...
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_LessonReviewerVideo_ServiceScheduleId]') AND type = 'D')
BEGIN
...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_LessonReviewerVideo_Duration]') AND type = 'D')
BEGIN
