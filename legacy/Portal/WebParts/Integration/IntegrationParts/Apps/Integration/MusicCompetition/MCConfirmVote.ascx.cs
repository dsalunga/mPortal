...
{
    public partial class MCConfirmVote : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (id > 0 && !string.IsNullOrEmpty(code))
                {
                    ...
                    if (vote != null && vote.CompetitionId > 0 && (competition = MCCompetition.Provider.Get(vote.CompetitionId)) != null && !competition.IsVoteLocked)
                    {
                        if (vote.Status == 0 && vote.Code.Equals(code, StringComparison.InvariantCultureIgnoreCase))
                        {
                            // Valid request
