...
CREATE TABLE [dbo].[LessonReviewerSession](
	...
	[CouncillorNotes] [nvarchar](max) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,
	[CouncillorUserId] [int] NOT NULL,
	[Status] [int] NOT NULL,
...
BEGIN
ALTER TABLE [dbo].[LessonReviewerSession] ADD  CONSTRAINT [DF_LessonReviewerSession_RejectReason]  DEFAULT ('') FOR [CouncillorNotes]
END

GO
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[DF_Table1_AssignedCouncillorId]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[LessonReviewerSession] ADD  CONSTRAINT [DF_Table1_AssignedCouncillorId]  DEFAULT ((-1)) FOR [CouncillorUserId]
END
