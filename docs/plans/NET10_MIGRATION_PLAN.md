...
- [x] Resolve solution restore/build blockers — `EnableWindowsTargeting=true` added to `Directory.Build.props` (2026-04-09); all NuGet version conflicts resolved; `dotnet build mPortal.slnx` succeeds with 0 errors.
- [x] Align EF Core package versions — upgraded from 9.0.x to **EF Core 10.0.5** across BranchLocator, WCMS.Framework, and Integration (2026-04-09). Also upgraded: Npgsql.EntityFrameworkCore.PostgreSQL 10.0.1, Microsoft.Data.SqlClient 6.1.3, Npgsql 10.0.2, SMO 181.19.0, System.Configuration.ConfigurationManager 10.0.5.
- [x] Upgrade vulnerable `SixLabors.ImageSharp` — upgraded from 3.1.7 to **3.1.12** across 3 projects (2026-04-09), resolving `NU1902`.
...
- [x] Re-baseline analyzer/compile warning debt — root `.editorconfig` created (2026-04-09) triaging 2590 pre-existing warnings: 14 security (CA5xxx) remain as warnings; all style/naming/nullable rules lowered to suggestion severity. Clean build: **0 errors, 17 warnings**.

**Compile errors resolved (2026-04-09):**
- Removed legacy `HttpRequestBase` overloads from `Core/WCMS.Common/QueryParser.cs`.
- Switched `System.Data.SqlClient` → `Microsoft.Data.SqlClient` in MySQL TableEditor.
- Created `PathMapper.cs` in `Core/WCMS.Common/Utilities/` (was missing).
- Added `ConfigUtil.GetConnectionString()` method.
- Added `DbHelper.ExecuteScalar(connString, cmdText, params)` overload.
- Disambiguated SMO `Index` type in MySQL TableEditor `Form1.cs`.
...
- [x] Verify database-resolved page rendering and ViewComponent output against seeded page/template datasets (full E2E) — covered by `PostgreSqlProviderIntegrationTests` (`CmsFallback_RootPath_RendersSeededPage`) with seeded `Portal/Assets/Database/PostgreSQL/seed-data.sql` + `seed-test-fixtures.sql`.
