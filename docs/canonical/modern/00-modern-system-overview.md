...
- Data: `EF Core 10` + `DbUp` hybrid migration flow with `PostgreSQL` as default RDBMS (`DbUp.Postgresql`), and SQL Server as a compatibility lane where required.
...
- Delivery: `GitHub Actions` as target CI/CD platform (workflows currently disabled in private repo until release hardening phase).
